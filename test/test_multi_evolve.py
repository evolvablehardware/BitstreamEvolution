...
class TestEvolution(Evolution):
    ...
    def assert_evolve_call_contains(self, callIndex, args: list[(str, any)]):
        ...
        for argument in args:
            if argument[0] not in call_argument_dict:
                raise IndexError(
                    f"Argument '{argument[0]}' not found in evolve call. Available arguments are: '{call_argument_dict.keys()}'"
                )

            assert argument[1] == call_argument_dict[argument[0]], (
                f"Argument '{argument[0]}' did not match on evolve call index {callIndex}. {args[argument]} != {call_argument_dict[argument]}."
            )
...
def test_evolve_list_of_configs(test_evolution_object) -> Iterator[Callable]:
    test_evolve_list_of_configs = partial(
        multi_evolve.evolve_list_of_configs_selecting_evolution,
        evolution_object=test_evolution_object,
    )
    yield test_evolve_list_of_configs


def test_check_call_count_on_mock(
    test_evolution_object: TestEvolution, test_evolve_list_of_configs: Callable
):
...
@pytest.mark.parametrize("configName", ["config1", "config2", "config3", "config4"])
def test_single_config_recieved(
    configName: str, test_evolution_object: TestEvolution, test_evolve_list_of_configs: Callable
):
    ...
    test_evolution_object.assert_last_evolve_call_contains(
        [
            (TestEvolution.PRIMARY_CONFIG_PATH, configName),
...
def test_multiple_config_recieved(
    configs_name: [str], test_evolution_object: TestEvolution, test_evolve_list_of_configs: Callable
):
    ...
    test_evolution_object.assert_all_calls_contain(
        [
            (TestEvolution.BASE_CONFIG_PATH, "base-config"),
            (TestEvolution.OUTPUT_DIRECTORY, "output-directory"),
            (TestEvolution.EXPERIMENT_DESCRIPTION, "experiment-description"),
    ...
    configs_evolved: list[str] = list(
        map(
            lambda call: call[TestEvolution.PRIMARY_CONFIG_PATH],
            test_evolution_object.all_evolve_call_arguments(),
        )
    )

    assert test_evolution_object.get_evolve_call_count() == len(configs_name), (
        "Incorrect number of calls (" + str(test_evolution_object.get_evolve_call_count()) + ")"
    )
    for i in range(len(configs_name)):
        assert configs_name[i] == configs_evolved[i], (
            "Error: " + str(configs_name) + "!=" + str(configs_evolved)
        )
