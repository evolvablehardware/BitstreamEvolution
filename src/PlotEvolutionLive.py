...
def run():
    ...
    def animate_generation(i):
        ...
        ax1.hlines(
            y=avg,
            xmin=0,
            xmax=config.get_population_size() + 1,
            color="violet",
            linestyles="dotted",
        )
        ax1.scatter(
            xs, ys, color=((accent_color2 + "dd") if is_transparent else (accent_color2 + "ff"))
        )
        if config.is_pulse_func():
            ...
            if config.is_pulse_count():
                ax1.hlines(
                    y=config.get_desired_frequency(),
                    xmin=1,
                    xmax=config.get_population_size(),
                    color="red",
                    linestyles="dotted",
                )
            ax1.set_ylim([0, None])
        ...
        if formal:
            ax1.legend(
                ["Average Fitness", "Individual Fitness", "Target Fitness"],
    ...
    def animate_epoch(i):
        ...
        if formal:
            ax2.legend(
                plots, labels, bbox_to_anchor=(1.15, 0.5), loc="center left", borderaxespad=0
            )
    ...
    def animate_waveform(i):
        ...
        if formal:
            ax4.legend(
                ["Trigger Voltage", "Circuit Voltage"],
    ...
    def animate_pops(i):
        ...
        if len(ys) > 0:
            ...
            ax6.set(
                xlabel="Generation",
                ylabel="Number from Population",
                title="Circuits from Each Source Population",
            )
    ...
    def anim_violin_plots(i):
        ...
        for _ in range(0, len(collections)):
    ...
    has_st_plot = False
    if (
        config.get_simulation_mode() == "FULLY_INTRINSIC" and not config.is_pulse_func()
    ) or config.get_simulation_mode() == "FULLY_SIM":
        rows = rows + 1
        has_wf_plot = True

    if (
        config.get_simulation_mode() == "FULLY_INTRINSIC"
        and config.get_fitness_func() == "TONE_DISCRIMINATOR"
    ):
        rows = rows + 1
        has_st_plot = True
