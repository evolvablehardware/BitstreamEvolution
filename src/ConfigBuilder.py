...
class ConfigBuilder:
    ...
    def build_config(self, output):
        ...
        """
        config_values, files_list = self.__get_config_values_from_file(
            self.__input, self.__override_base_config
        )
    ...
    def __get_config_values_from_file(self, input, override_base_config=None):
        ...
        if base_config_path is not None:
            base_values, base_config_files_list = self.__get_config_values_from_file(
                base_config_path
            )
    ...
    def __get_comment_for_param(self, section, param, config_lines):
        ...
        for i in range(section_line_index + 1, len(config_lines)):
            line = config_lines[i]
            if re.search(r"^" + re.escape(param) + r"\s*\=.*$", line) is not None:
            ...
            for name, config_values in sections.items():
                ...
                for config_value in config_values:
                    ...
                    file.write(
                        config_value.param
                        + " = "
                        + self.__escape_config_value(config_value.value)
                        + "\n"
                    )
