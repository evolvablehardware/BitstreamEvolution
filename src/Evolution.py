...
class Evolution:
    ...
    def evolve(
        ...
        if print_action_only:
            ...
            print(
                f"Config: {primary_config_path}, Base Config: {base_config_path}, Output Directory: {output_directory}, Experiment Description: {experiment_description}."
            )
        ...
        if config.get_upload_to_arduino():
            _compile_result = run(
                [
                    ...
                    "-b",
                    "arduino:avr:nano",
                    "data/ReadSignal/ReadSignal.ino",
                ]
            )
        ...
        if config.get_simulation_mode() == "FULLY_INTRINSIC":
            ...
            run(["iceprog", "-d", "i:0x0403:0x6010:0", "data/hardware_blink.bin"])
    ...
    def validate_arguments(self, output_directory):
        ...
        if (output_directory is not None) and (not os.path.isdir(output_directory)):
            invalid_info += (
                f"PATH_NOT_RECOGNIZED: Output directory not recognized: {output_directory}\n"
            )
