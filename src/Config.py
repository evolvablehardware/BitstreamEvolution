...
class Config:
    ...
    def get_desired_frequency(self):
        ...
        if desiredFreq < 0:
            self.__log_error(
                1, "Invalid desired frequency " + str(desiredFreq) + "'. Must be greater than zero."
            )
            exit()
        return desiredFreq
    ...
    def get_num_samples(self):
        ...
        if value < 1:
            self.__log_error(
                1, "Invalid number of samples " + str(value) + "'. Must be greater than zero."
            )
            exit()
        return value
    ...
    def get_num_passes(self):
        ...
        if value < 1:
            self.__log_error(
                1, "Invalid number of passes " + str(value) + "'. Must be greater than zero."
            )
            exit()
        return value
    ...
    def get_population_size(self):
        ...
        if popSize < 1:
            self.__log_error(
                1, "Invalid population size " + str(popSize) + "'. Must be greater than zero."
            )
            exit()
        return popSize
    ...
    def get_mutation_probability(self):
        ...
        if prob < 0.0:
            self.__log_error(
                1, "Invalid mutation probability " + str(prob) + "'. Must be greater than zero."
            )
            exit()
        if prob > 1.0:
            self.__log_error(
                1, "Invalid mutation probability " + str(prob) + "'. Must be less than one."
            )
            exit()
        return prob
    ...
    def get_crossover_probability(self):
        ...
        if prob < 0.0:
            self.__log_error(
                1, "Invalid crossover probability " + str(prob) + "'. Must be greater than zero."
            )
            exit()
        if prob > 1.0:
            self.__log_error(
                1, "Invalid crossover probability " + str(prob) + "'. Must be less than one."
            )
            exit()
        return prob
    ...
    def get_elitism_fraction(self):
        ...
        if frac < 0.0:
            self.__log_error(
                1, "Invalid elitism fraction " + str(frac) + "'. Must be greater than zero."
            )
            exit()
        if frac > 1.0:
            self.__log_error(
                1, "Invalid elistism probability " + str(frac) + "'. Must be less than one."
            )
            exit()
        return frac
    ...
    def get_random_injection(self):
        ...
        if frac < 0.0:
            self.__log_error(
                1, "Invalid random injection rate " + str(frac) + "'. Must be greater than zero."
            )
            exit()
        if frac > 1.0:
            self.__log_error(
                1, "Invalid random injection rate " + str(frac) + "'. Must be less than one."
            )
            exit()
        return frac
    ...
    def get_randomize_threshold(self):
        ...
        if threshold < 0:
            self.__log_error(
                1, "Invalid random threshold " + str(threshold) + "'. Must be greater than zero."
            )
            exit()
        return threshold
    ...
    def get_target_fitness(self):
        ...
        if targetFitness < 0.0:
            self.__log_error(
                1, "Invalid target fitness " + str(targetFitness) + "'. Must be greater than zero."
            )
            exit()
    ...
    def get_test_circuit(self):
        try:
            ...
            self.__log_error(
                1,
                "Invalid file path "
                + self.get_sensitivity_parameters("TEST_CIRCUIT")
                + " for test circuit.",
            )
    ...
    def get_sensitivity_trials(self):
        ...
        if trials < 1:
            self.__log_error(
                1,
                "Invalid number of sensitivity trials"
                + str(trials)
                + "'. Must be greater than zero.",
            )
            exit()
    ...
    def get_sensitivity_time(self):
        try:
            date_time = datetime.strptime(
                self.get_sensitivity_parameters("SENSITIVITY_TIME"), "%j:%H:%M:%S"
            )
            seconds = (
                86400 * date_time.day
                + 3600 * date_time.hour
            ...
            self.__log_warning(
                1,
                "Invalid value for the amount of time to sensitivity trials. Should be in the format %-j:%H:%M:%S. Program will not terminate based on the amount of time passed",
            )
            return "IGNORE"
        if seconds < 0:
            self.__log_error(
                1,
                "Invalid amount of time to do sensitivity trials: "
                + str(seconds)
                + "'. Must be greater than zero.",
            )
            exit()
    ...
    def get_transfer_interval(self):
        ...
        if interval < 1:
            self.__log_error(
                1,
                "Invalid transfer interval size " + str(interval) + "'. Must be greater than zero.",
            )
            exit()
        return interval
    ...
    def get_population_bistream_save_interval(self):
        ...
        if interval < 1:
            self.__log_error(
                1,
                "Invalid population bistream save interval "
                + str(interval)
                + "'. Must be greater than zero.",
            )
            exit()
    ...
    def validate_all(self):
        ...
        if (
            self.get_fitness_func() == "PULSE_CONSISTENCY"
            and (self.get_num_passes() * self.get_num_samples()) <= 1
        ):
            self.__log_error(
                1, "PULSE_CONSISTENCY function can only be used with multiple samples/passes"
            )
            exit()
        # MAP elites can only be used with VARIANCE, COMBINED, and PULSE CONSISTENCY
        if self.get_selection_type() == "MAP_ELITES" and self.get_fitness_func() not in [
            "VARIANCE",
            "COMBINED",
            "PULSE_CONSISTENCY",
        ]:
            self.__log_error(
                1,
                "MAP_ELITES selection can only be used with the following fitness functions: "
    ...
    def is_pulse_func(self):
        return (
            self.get_fitness_func() == "PULSE_COUNT"
            or self.get_fitness_func() == "TOLERANT_PULSE_COUNT"
            or self.get_fitness_func() == "SENSITIVE_PULSE_COUNT"
            or self.get_fitness_func() == "PULSE_CONSISTENCY"
        )
