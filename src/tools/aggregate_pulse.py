...
diversities = []
with open(BEST_LIVE_DATA_PATH) as file:
...
with open(BEST_LIVE_DATA_PATH, "r+") as best_file:
    ...
    best_file.write(f"0, 0, 0, 0, 0, {diversities[0]!s}\n")
    with open(PULSE_LIVE_DATA_PATH) as pulse_file:
        ...
        for pulse_line in pulse_lines:
            ...
            best_file.write(
                f"{generation!s}, {best!s}, {worst!s}, {avg!s}, {best_overall_fitness!s}, {diversity!s}\n"
            )
