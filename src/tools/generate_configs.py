...
old_sensitivity_config_base = """[TOP-LEVEL PARAMETERS]
...
def old_sensitivity_config_generator() -> Generator[CommandInfo, None, None]:
    for circuit_id in range(10, 510, 10):
        ...
        with open(config_path, "w") as config_file:
            config_file.write(
                old_sensitivity_config_base.format(
                    base_config_path=base_config_path, circuit_id=circuit_id
                )
            )

        yield CommandData(
            config_path=config_path,
            description=f"Runing sensitivity experiment on {circuit_id}.ini",
        )


################################## PULSE COUNT SCRIPTS ################################

pulse_count_config_base = """[TOP-LEVEL PARAMETERS]
base_config = {base_config_path}
...
def pulse_count_config_generator(
    ...
    skip_next_if_skipped: bool = False,
    ...
    def create_config(
        target_pulse_count: int, fitness_funciton: str, population_size: int, max_generations: int
    ) -> str:
        # Generate the path for each pulse count
        config_path = os.path.join(
            generated_configs_dir, f"{target_pulse_count}_with__{fitness_funciton}.ini"
        )

        with open(config_path, "w") as config_file:
            config_file.write(
                pulse_count_config_base.format(
...
pulse_count_sensitivity_config_base = """[TOP-LEVEL PARAMETERS]
base_config = {base_config_path}
...
def pulse_count_then_sensitivity_config_generator(
    target_pulses: list[int],
    use_tolerant_ff: bool = True,
    use_sensitive_ff: bool = True,
    population_size: int = 50,
    ...
    def create_config_pair(
        target_pulse_count: int, fitness_funciton: str, pop_size: int, max_gens: int
    ) -> Generator[CommandInfo, None, None]:
        ...
        pc_gen = pulse_count_config_generator(
            target_pulses=[target_pulse_count],
        ...
        if pulse_count_experiment.copy_best_asc_target_path is None:
            raise ValueError(
                "The pulse count must output to a target path for Sensitivity to test it."
            )

        ## Generate the config for the Sensitivity run.
        config_path = os.path.join(
            generated_configs_dir,
            f"Sensitivity_For_Pulse_Count_of_{target_pulse_count}_with_{fitness_funciton}.ini",
        )

        ## UPDATE SO TO GENERATE SENSITIVITY CONFIG 120 mins
        with open(config_path, "w") as config_file:
            config_file.write(
                pulse_count_sensitivity_config_base.format(
                    base_config_path=base_config_path,
                    fitness_function=fitness_funciton,
                    desired_frequency=target_pulse_count,
...
config_generator: Generator[CommandInfo, None, None] = (
    pulse_count_then_sensitivity_config_generator(
        target_pulses=[1000, 2000, 4000],
        use_tolerant_ff=True,
...
evolve_command_base = (
    "python3 src/evolve.py -c {config_path} -d {description} -o {output_directory}"
)
    ...
    for command_data in config_generator:
        ...
        bash_file.write(
            bash_command_wrapper_logic.format(
                ...
                action_if_failure="SkipNextCommand=1"
                if command_data.skip_next_command_if_error
                else "SkipNextCommand=0",
                action_if_success=(
                    "SkipNextCommand=1"
                    if command_data.skip_next_command_if_success
                    else "SkipNextCommand=0"
                )
                + (
                    f"\n\tcp {path_best_asc_in_workspace} {command_data.copy_best_asc_target_path}"
                    if command_data.copy_best_asc_target_path is not None
                    else ""
                ),
                action_if_skipped="SkipNextCommand=1"
                if command_data.skip_next_command_if_skipped
                else "SkipNextCommand=0",
            )
        )
...
try:
    os.chmod(
        generated_bash_script_path,
        stat.S_IREAD | stat.S_IWRITE | stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO,
    )
