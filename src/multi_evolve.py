...
from evolve import BUILT_CONFIG_PATH
...
parser = argparse.ArgumentParser(
    prog=program_name, description=program_description, epilog=program_epilog
)
parser.add_argument("-c", "--configs", type=str, nargs="*", default=default_config_array, help="")
parser.add_argument(
    "-bc",
    "--base-config",
    type=str,
    default=default_base_config,
    help="The config any unspecified values in the main config is pulled from. "
...
def evolve_list_of_configs_selecting_evolution(
    *configs: str,
    base_config: str,
    output_directory: str,
    experiment_description: str,
    print_action_only: bool = False,
    evolution_object: Evolution | None = None,
):
    ...
    for config in configs:
        ...
        if print_action_only:
            print(
                f"multi-evolve:{{config:{config},output:{output_directory},base_config:{base_config},description:{experiment_description}}}"
            )
